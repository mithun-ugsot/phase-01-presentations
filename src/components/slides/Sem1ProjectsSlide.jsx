...
import { SEM1_PROJECTS } from '../../data/trackData'

export default function Sem1ProjectsSlide({ direction }) {
  const projectCount = SEM1_PROJECTS.names.length

  return (
    <SlideFrame direction={direction}>
      <SlideCard
        eyebrow="Project Portfolio"
        title="Semester 1 Projects Overview"
        ...
        <div className="grid gap-3 md:grid-cols-[minmax(0,1.05fr)_minmax(0,0.95fr)] md:gap-4">
          <div className="space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <div>
                <p className="text-xs font-semibold text-slate-900">Projects in Sem 01</p>
                <p className="text-[10px] text-slate-500">From tables & forms to product UI clones</p>
              </div>
            </div>

            <div className="grid grid-cols-3 gap-1.5">
              {SEM1_PROJECTS.featured.map((project, i) => (
                <motion.figure
                  key={project.name}
            ...
            <div className="flex flex-wrap gap-1">
              {SEM1_PROJECTS.topics.map((topic) => (
                <span
                  key={topic}
          ...
          <div className="flex min-h-0 flex-col rounded-lg border border-slate-200 bg-slate-50/80 p-2.5">
            ...
            <ul className="grid min-h-0 flex-1 grid-cols-2 gap-x-2 gap-y-0.5 overflow-hidden lg:grid-cols-2">
              {SEM1_PROJECTS.names.map((name, i) => (
                <motion.li
                  key={name}
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
